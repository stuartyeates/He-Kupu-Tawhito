...
JAVA_OPTS=-"Xmx4000m -Dfile.encoding=UTF-8"
JAVA_OPTIONS="-Xmx4000m -Dfile.encoding=UTF-8"
CLIENT_JAVA_OPTIONS="-Xmx4000m -Dfile.encoding=UTF-8"
...
$(EXIST_HOME): $(EXIST_ZIP)
	...
	cat $(EXIST_HOME)/bin/functions.d/eXist-settings.sh | sed 's/-Xmx512m/-Xmx4000m/' > tmp.settings
...
#	$(EXIST_HOME)/bin/client.sh -m /db/he_kupu_tawhito/ -p korero/www.nzetc.org/*.words.xml
